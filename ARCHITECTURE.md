...
> **Delivered state (current build).** The shipped server implements the full service surface below as Go [Connect](https://connectrpc.com/) services (`RegistryService`, `TrustService`, `ConfigService`, `IngestService`, `QueryService`, `WebhookService`) over HTTP/2. Event ingest defaults to an **in-process channel broker + batched async writer** into an **in-memory analytics store**, with a **production data plane now delivered behind the same `Broker`/`AnalyticsStore`/`EventSink` interfaces**: a **Kafka/Redpanda** broker (at-least-once, tenant-partitioned, idempotent producer + load-shedding) and a **ClickHouse** analytics + raw-event store (`ReplacingMergeTree` → effectively-once at query time, `tenant_id`-leading `ORDER BY` for physical per-tenant isolation, monthly partitions + TTL/retention). Backends are selected via `KSEAL_BROKER` (`memory`|`kafka`) / `KSEAL_ANALYTICS` (`memory`|`clickhouse`) — default off, fail-closed on misconfiguration, callers and all `QueryService` wire shapes unchanged (see `server/data-plane/ingest/`, `docs/data-plane-scale.md`). The transactional source of truth is **Postgres 16** (with row-level-security tenant isolation), and **Redis 7** backs trust-session lookups and rate limits. Signing keys are sealed with AES-256-GCM envelope encryption under a 32-byte KEK via the `TenantSealer` seam: the platform KEK is the default, and **customer-managed keys (BYOK)** are delivered as a per-tenant KMS-wrapped DEK (`CMKKeyManager`, self-describing `KSC1` envelope), fail-closed on KMS error or disabled-CMK open, gated by `KSEAL_CMK_KMS_URI` (default off). Additional hardening ships behind default-off env vars: Redis TLS/AUTH (`KSEAL_REDIS_TLS`/`_PASSWORD`/`_CA_FILE`), an OTLP exporter emitting real spans + metrics across the ingest/query/attestation hot paths (`KSEAL_OTLP_ENDPOINT`/`_SAMPLE_RATIO`; `kseal.ingest.*`/`kseal.broker.*` metrics), and per-tenant raw-event retention (`KSEAL_RAW_RETENTION_DAYS` + per-tenant `raw_retention_days`, with a tenant-isolated purge routine). Webhook fan-out is delivered with HMAC-SHA256 signing, retries, and per-endpoint circuit breaking. SIEM export is delivered (`server/data-plane/siem`): per-tenant connectors for Splunk HEC, Microsoft Sentinel, and Elastic with a backpressured, batched, at-least-once exporter (per-tenant circuit breaker + privacy allow-list), plus connector templates. The **`ComplianceService`** (`server/control-plane/compliance`) adds the enterprise trust & compliance surface: a hash-chained, tamper-evident, tenant-scoped append-only **audit trail** (with `VerifyAuditChain`), a machine-readable **data-processing registry**, an Ed25519-signed **kill switch** with monotonic anti-rollback versioning (serialized per scope via `pg_advisory_xact_lock`, delivered over the signed-config channel), and **canary rollout + guardrail-driven auto-rollback** with deterministic tenant/app/instance bucketing. Every RPC is additive to the v1 wire contract, requires a valid API key, and filters on the caller's tenant; new features are flag-gated and fail-safe (default off). A per-tenant **dedicated/regulated tier** is keyed off an HKDF per-tenant key domain.
>
...
| Streaming / ingest | Kafka / Redpanda | In-process channel broker (default) **or** Kafka/Redpanda broker — `KSEAL_BROKER=kafka`, default off |
| Analytics store | ClickHouse | In-memory store (default) **or** ClickHouse store — `KSEAL_ANALYTICS=clickhouse`, default off |
...
| Observability | OpenTelemetry | Prometheus metrics at `/metrics`; `/healthz` + `/readyz` health checks; OTLP real spans + metrics on ingest/query/attestation hot paths (`KSEAL_OTLP_ENDPOINT`, default off) |
