...
>
> **Console/CLI wiring (in progress).** The compliance/ops console views (`web/console`) and the `kseal` CLI compliance commands currently read these surfaces through a stream-local proto with graceful degradation (they render "not available yet" against a server that doesn't expose the RPC). Re-pointing them onto the canonical `kseal.v1.ComplianceService` client — now live on `main` — is the tracked remaining integration step; the stream-local and canonical message shapes need reconciliation first.
