...
echo "kseal-partner-console: rendered ${TARGET} (apiBaseUrl='${API_BASE_URL}')"
...
if [ -n "${API_BASE_URL}" ]; then
  ...
  if printf '%s' "${ORIGIN}" | grep -Eq '^https?://[A-Za-z0-9._-]+(:[0-9]+)?$'; then
    ...
    echo "kseal-partner-console: WARNING KSEAL_API_BASE_URL is not a clean http(s) origin; leaving connect-src permissive"
  fi
fi
...
if [ -f "${CSP_SNIPPET}" ]; then
  ...
  echo "kseal-partner-console: set CSP connect-src to '${CONNECT_SRC}'"
fi
