...
///
/// The build-tool plugin runs inside SwiftPM's sandbox where pulling an external
/// crypto package would add a fetch step and a network dependency at resolve
/// time. SHA-256 is stable and easy to verify against the published test
/// vectors (see `SHA256Tests`), so kseal vendors it here to keep the hardening
/// toolkit zero-dependency and fully offline. This hashing is used only for
/// build-provenance digests (seed digest, build hash, keystream derivation) —
/// never for protecting user data, which stays in the audited Rust core.
public enum SHA256 {
    ...
    public static func hash(_ message: [UInt8]) -> [UInt8] {
        ...
        // Pre-processing: pad to a multiple of 64 bytes with the 1-bit, zeros,
    ...
    /// Convenience overload hashing the UTF-8 bytes of `string`.
    public static func hash(_ string: String) -> [UInt8] {
        hash(Array(string.utf8))
    }

    /// Returns the lowercase hex encoding of the SHA-256 digest of `message`.
    public static func hexDigest(_ message: [UInt8]) -> String {
        HexEncoding.encode(hash(message))
    }

    /// Returns the lowercase hex encoding of the SHA-256 digest of `string`.
...
/// Lowercase hex encode/decode helpers used across the toolkit.
public enum HexEncoding {
    ...
    public static func decode(_ string: String) -> [UInt8]? {
        let chars = Array(string.lowercased().utf8)
